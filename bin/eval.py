...
from toolkit.datasets import OTBDataset, ShipDataset, OccShipDataset, UAVDataset, LaSOTDataset, \
...
parser.add_argument('--dataset', '-d', default='OccShip', type=str,
                    help='dataset name')
...
def main():
    ...
    elif 'Ship' == args.dataset:  # ship dataset
        dataset = ShipDataset(args.dataset, root)
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        success_ret = {}
        with Pool(processes=args.num) as pool:
    ...
    elif 'OccShip' == args.dataset:   # occlusion attribute ship dataset
        dataset = OccShipDataset(args.dataset, root)
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        success_ret = {}
        with Pool(processes=args.num) as pool:
            for ret in tqdm(pool.imap_unordered(benchmark.eval_success,
                trackers), desc='eval success', total=len(trackers), ncols=50):
                success_ret.update(ret)
        precision_ret = {}
        with Pool(processes=args.num) as pool:
            for ret in tqdm(pool.imap_unordered(benchmark.eval_precision,
                trackers), desc='eval precision', total=len(trackers), ncols=50):
                precision_ret.update(ret)
        benchmark.show_result(success_ret, precision_ret,
                show_video_level=args.show_video_level)
        if args.vis:
